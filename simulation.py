...
cf_param_functions = {
    ...
    "SpeedMode": lambda traci_, vehID, SpeedMode: traci_.vehicle.setSpeedMode(
        vehID, 32 if vehID == "leader" else 31
    ),
}
...
def config_create(run_id, cf_parameters):
    ...
    config_data = {
        ...
        "SUMOParameters": {"step": 0.1, "delay": 3},
    }
...
def run(step, delay, leader_dict, cf_parameters):
    ...
    for row in leader_dict:
        if (symtime >= delay) and not add_flag:
            add_vehicle(
                "follower" if symtime > 0 else "leader", start_speed, cf_parameters
            )
            add_flag = True
    ...
    if __name__ == "__main__":
        ...
        traci.start(
            [
                sumoBinary,
                "-n",
                f"{ROOT / 'sumo-xml' / 'net.net.xml'}",
                "--step-length",
                f"{config.SUMOParameters.step}",
                "--no-step-log",
                "true",
                "--fcd-output",
                f"{ROOT / 'sumo-xml' / 'output' / f'{run_id}_fcd.xml'}",
                "--fcd-output.acceleration",
            ]
        )
        run(
            config.SUMOParameters.step,
            config.SUMOParameters.delay,
            input_data(config.Config.leader_file),
            config.CFParameters,
        )
...
def main():
    ...
    all_combinations = list(
        itertools.product(tau, acceleration, deceleration, speed_factor)
    )
...
if __name__ == "__main__":
    ...
    run_ids = main()
    print(
        f"{run_ids} simulations finished --- %s seconds ---"
